...
export const educationHistorySchema = z.object({
    ...
    nisn: z.string().length(10, "NISN harus 10 digit").regex(/^\d+$/, "NISN harus berupa angka").optional().or(z.literal("")),
});

export const parentDataSchema = z.object({
    nama_ayah: z.string().min(3, "Nama ayah harus diisi"),
    nik_ayah: z.string().length(16, "NIK ayah harus 16 digit").regex(/^\d+$/, "NIK harus berupa angka").optional().or(z.literal("")),
    ...
    nik_ibu: z.string().length(16, "NIK ibu harus 16 digit").regex(/^\d+$/, "NIK harus berupa angka").optional().or(z.literal("")),
